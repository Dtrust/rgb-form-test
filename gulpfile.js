...
const path = {
	...
	//This is temp special for RGB-Form
	utils: {
		src: `${sourceDir}/utils/*.js`,
		dest: `${baseDir}/js`,
	},
	fonts: {
		src: `${sourceDir}/fonts/*.*`,
...
function transferUtils(cb) {
	return src(path.utils.src)
	.pipe(dest(path.utils.dest));
	cb();
};
...
function watchProject(cb) {
	...
	watch([path.img.dir], optimizeIMG), watch([path.media.src], transferMedia);
	watch([path.favicon.src], transferFavicon);
	watch([path.utils.src], transferUtils);
